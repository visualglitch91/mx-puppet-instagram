...
export class Instagram {
  ...
  public async newPuppet(puppetId: number, data: any) {
    ...
    client.on("reel_share", async (msg: any, share: any) => {
      ...
      const pronoun =
        share.media.user.pk === this.puppets[puppetId].data.userId
          ? "your"
          : "their";
    ...
    client.on("media_share", async (msg: any, share: any) => {
      ...
      await this.puppet.sendMessage(params, {
        ...
        formattedBody: `New media by <a href="${escapeHtml(
          userWebUrl,
        )}">${escapeHtml(user)}</a> has been shared: <a href="${escapeHtml(
          mediaWebUrl,
        )}">${escapeHtml(mediaWebUrl)}</a>`,
      });
      await this.puppet.sendFileDetect(params, imgUrl);
      await this.puppet.sendMessage(params, {
        body: "> " + share.caption.text,
        formattedBody: `<blockquote>${escapeHtml(
          share.caption.text,
        )}</blockquote>`,
      });
    });
